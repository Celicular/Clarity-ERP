...
async function migrate() {
  ...
  try {
    ...
    `);
    console.log("   ✓ ticket_replies table");

    /* ── 41. webmail_credentials ── */
    await client.query(`
      CREATE TABLE IF NOT EXISTS webmail_credentials (
        id          TEXT PRIMARY KEY,
        user_id     TEXT UNIQUE NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        email       TEXT NOT NULL,
        password    TEXT NOT NULL,
    ...
    `);
    console.log("   ✓ webmail_credentials table");

    /* ── 42. webmail_contacts ── */
    await client.query(`
      CREATE TABLE IF NOT EXISTS webmail_contacts (
        id          TEXT PRIMARY KEY,
        user_id     TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        email       TEXT NOT NULL,
        name        TEXT,
    ...
    `);
    console.log("   ✓ webmail_contacts table");
